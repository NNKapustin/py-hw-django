...
def test_get_course_filter_id(api_client, course_factory):
    ...
    response = api_client.get('/api/v1/courses/?id=3')
    assert response.status_code == 200
    data = response.json()
    assert data[0]['id'] == 3
...
def test_get_course_filter_id(api_client, course_factory):
    courses = course_factory(_quantity=10)
    response = api_client.get(f'/api/v1/courses/?name={courses[1].name}')
    assert response.status_code == 200
    data = response.json()
...
def test_create_course(api_client, course_factory, student_factory):
    ...
    response = api_client.post('/api/v1/courses/', data={'name': 'course1', 'students': []}, format='json')
...
def test_change_course(api_client, course_factory, student_factory):
    ...
    response = api_client.patch(f'/api/v1/courses/{course[0].id}/', data={'name': new_name}, format='json')
    assert response.status_code == 200
...
def test_course_is_full(course_factory, student_factory, settings):
    students = student_factory(_quantity=settings.MAX_STUDENTS_PER_COURSE)
